...
namespace TransparentClock
{
    public class DashboardForm : Form
    {
        ...
        private readonly ComboBox focusDayComboBox;
        private readonly ListView focusHourlyListView;
        ...
        public DashboardForm()
        {
            ...
            focusLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 55));
            focusLayout.RowStyles.Add(new RowStyle(SizeType.AutoSize));
            focusLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 45));
            ...
            breakdownTopPanel.Controls.Add(focusDayComboBox);

            focusHourlyListView = new ListView
            {
                Dock = DockStyle.Fill,
                View = View.Details,
                FullRowSelect = true
            };
            focusHourlyListView.Columns.Add("Hour", 60);
            focusHourlyListView.Columns.Add("Bar", 120);
            focusHourlyListView.Columns.Add("Minutes", 80);

            focusLayout.Controls.Add(focusHeader, 0, 0);
            focusLayout.Controls.Add(focusHistoryPanel, 0, 1);
            focusLayout.Controls.Add(breakdownTopPanel, 0, 2);
            focusLayout.Controls.Add(focusHourlyListView, 0, 3);
            ...
            focusDayComboBox.SelectedIndexChanged += (_, __) => RefreshDailyBreakdown();
        ...
        private void RefreshFocusHistoryList()
        {
            ...
            var entryMap = entries.ToDictionary(item => item.Date, item => item.TotalFocusMinutes);

            string? previousSelection = focusDayComboBox.SelectedItem as string;

            focusDayComboBox.Items.Clear();
            foreach (var entry in entries.OrderBy(item => item.Date))
            {
                focusDayComboBox.Items.Add(entry.Date);
            }
            if (focusDayComboBox.Items.Count > 0)
            {
                if (!string.IsNullOrWhiteSpace(previousSelection) && focusDayComboBox.Items.Contains(previousSelection))
                {
                    focusDayComboBox.SelectedItem = previousSelection;
                }
                else
                {
                    string todayKey = DateTime.Today.ToString("yyyy-MM-dd");
                    focusDayComboBox.SelectedItem = focusDayComboBox.Items.Contains(todayKey) ? todayKey : focusDayComboBox.Items[focusDayComboBox.Items.Count - 1];
                }
            }
        ...
        private void RefreshSessionHistoryList()
        {
            ...
            {
                string dateText = entry.CompletedAt.ToString("yyyy-MM-dd");
                string timeText = entry.CompletedAt.ToString("HH:mm");
                string minutesText = entry.Minutes.ToString();
        ...
        {
            focusHourlyListView.Items.Clear();

            if (focusDayComboBox.SelectedItem is not string selected)
            {
                return;
            }

            if (!DateTime.TryParse(selected, out var date))
            {
                return;
            }
            ...
            for (int hour = 0; hour < 24; hour++)
            {
                ...
                int blocks = Math.Min(10, (int)Math.Round(minutes / 10.0));
                string bar = new string('█', blocks);

                var item = new ListViewItem(hour.ToString("00"));
                item.SubItems.Add(bar);
                item.SubItems.Add(minutes.ToString());
                focusHourlyListView.Items.Add(item);
            }
        }
